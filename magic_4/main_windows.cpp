    ...
    QMainWindow(parent)
{
    ...
    attribute->set_Position(69,69,1100,770);
...
void main_windows::show_pic(){
    ...
    hp_pw_df[11]->show();
    start=true;
    if(maps[flood][hero_x+1][hero_y]==64||maps[flood][hero_x-1][hero_y]==64
        ||maps[flood][hero_x][hero_y-1]==64||maps[flood][hero_x][hero_y+1]==64){
    if(meet_boss){
            meet_boss=false;
            if(fight_win==true){//打赢了
                for(int i=1;i<=8;i++){
                ...
                messageBox.setWindowTitle("很遗憾");
                messageBox.setText("您被魔王打败了，您必须重新开始，再次挑战");
                messageBox.show();
                // 定时器，3秒后隐藏消息框
                QTimer::singleShot(3000, &messageBox, &QMessageBox::hide);
